...
export default function ComponentAdminHandbookAdd(props) {
    ...
    const createHandbook = async () => {
        try {
            ...
            if (!handbookDetail.handbook_name.length || !handbookDetail.handbook_desc.length || !handbookDetail.handbook_img.length) {
                ...
                const handbookData = {handbook_name: handbookDetail.handbook_name, handbook_desc: handbookDetail.handbook_desc, handbook_img: handbookDetail.handbook_img }

                const addPostRes = await HandbookAPI.createNewHandbook(handbookData)

                if (addPostRes.data && addPostRes.data.success) {
                    setUpdateHandbookNoti({ status: true, noti: 'Thêm thông tin thành công', type: 'success' })
            ...
            <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}>
                            <Stack spacing={2} justifyContent="space-around" flexDirection={'row'}>
                                <Button variant="contained" sx={{ width: 'fit-content' }} onClick={() => createHandbook()}>Thêm cẩm nang</Button>
                            </Stack>
                        </Grid>
                    </Grid>
                </Grid>
