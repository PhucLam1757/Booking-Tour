...
export default function ComponentAdminServiceAddNew(props) {
    ...
    const createService = async () => {
        try {
            ...
            if (!serviceDetail.service_name.length || !serviceDetail.service_desc.length || !serviceDetail.service_img.length) {
                ...
                const postData = {service_name: serviceDetail.service_name, service_desc: serviceDetail.service_desc, service_img: serviceDetail.service_img }

                const addPostRes = await ServiceAPI.createNewService(postData)

                if (addPostRes.data && addPostRes.data.success) {
                    setUpdateServiceNoti({ status: true, noti: 'Thêm thông tin thành công', type: 'success' })
            ...
            <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}>
                            <Stack spacing={2} justifyContent="space-around" flexDirection={'row'}>
                                <Button variant="contained" sx={{ width: 'fit-content' }} onClick={() => createService()}>Thêm dịch vụ</Button>
                            </Stack>
                        </Grid>
                    </Grid>
                </Grid>
