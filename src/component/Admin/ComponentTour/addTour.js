...
export default function ComponentAdminAddTour(props) {
  ...
  const [addTourNoti, setAddTourNoti] = useState({
    status: false,
    noti: "",
    type: "",
  });
  ...
  const addNewTour = async () => {
    try {
      ...
      if (
        ...
        !tourData.date_go ||
        !tourData.date_return ||
        !tourData.place_from ||
        !tourData.place_to ||
        !tourData.tour_desc ||
        !tourData.tour_image ||
        !tourData.tour_policy ||
        !tourData.tour_transport ||
        !tourData.tour_type ||
        !tourData.about
      ) {
        setAddTourNoti({
        ...
        if (
          !/^-?\d+$/.test(tourData.adult_price) ||
          !/^-?\d+$/.test(tourData.child_price)
        ) {
          setAddTourNoti({
            status: true,
            noti: "Giá vé sai định dạng",
            type: "error",
          });
        }
        ...
        if (subCurrentDateAndDateGo <= 0) {
          setAddTourNoti({
            status: true,
            noti: "Ngày đi không thể nhỏ hơn ngày hiện tại",
            type: "error",
          });
        } else if (subDateGoAndDateReturn < 0) {
          setAddTourNoti({
            status: true,
            noti: "Ngày về không thể nhỏ hơn ngày đi",
            type: "error",
          });
        } else {
          ...
          setTourData({
            ...tourData,
            total_date: `${diffDays <= 0 ? diffDays + 1 : diffDays} ngày ${
              diffDays <= 0
                ? ""
                : diffDays - 1 > 0
                ? diffDays - 1 + " đêm"
          ...
          if (addTourRes.data && addTourRes.data.success) {
            setAddTourNoti({
              status: true,
              noti: "Thêm thông tin tour thành công",
              type: "success",
            });
            ...
            setAddTourNoti({
              status: true,
              noti: "Không thể thêm thông tin tour",
              type: "error",
            });
          }
        }
      ...
      <Grid item xs={12} md={6}>
        ...
        <Box sx={{ margin: "10px 0" }}>
          ...
          <div className="skeEditor-frame">
            <CKEditor
              ...
              data={tourData.tour_desc}
              onReady={(editor) => {
                console.log("Editor is ready to use!", editor);
              }}
              onChange={(event, editor) => {
                const data = editor.getData();
                setTourData({ ...tourData, tour_desc: data });
              }}
              config={{
                ckfinder: {
        ...
        <Box sx={{ margin: "10px 0" }}>
          ...
          <div className="skeEditor-frame">
            <CKEditor
              editor={ClassicEditor}
              data={tourData.tour_policy}
              onReady={(editor) => {
                console.log("Editor is ready to use!", editor);
              }}
              onChange={(event, editor) => {
                const data = editor.getData();
                setTourData({ ...tourData, tour_policy: data });
              }}
              config={{
                ckfinder: {
                  uploadUrl: "http://localhost:5005/api/tour/upload-tour-image",
          ...
          <FormControl fullWidth>
            ...
            <Select
              ...
              onChange={(event) => {
                setTourData({
                  ...tourData,
                  tour_transport: event.target.value,
                });
              }}
            >
          ...
          {addTourNoti.status && (
            <Grid item xs={12}>
              <Stack
                ...
                flexDirection={"row"}
              >
                <Alert severity={addTourNoti.type} sx={{ marginTop: "10px" }}>
                  {addTourNoti.noti}
                </Alert>
              </Stack>
            </Grid>
