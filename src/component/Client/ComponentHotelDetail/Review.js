...
export default function ComponentTourDetailReview(props) {
    ...
    const createNewReview = async () => {
        try {
            if(userData){
                if (userCanReview) {
                    const createReviewRes = await HotelAPI.createCustomerReview({ user_id: Number(userData.ctm_id), review: addReviewData, hotel_id: params.hotelId })
                    ...
                    setCreateReviewNoti({ status: true, noti: 'Bạn chỉ có thể review khi đã đặt khách sạn', type: 'error' })
                }
            }else{
                navigate('/login')
            }
    ...
    const checkUserCanReview = async () => {
        try {
            if (userData) {
                const checkRes = await HotelAPI.checkUserCanReview({ user_id: userData.ctm_id, hotel_id: params.hotelId })
    ...
    const getAllReview = async (page) => {
        try {
            const reviewRes = await HotelAPI.getReviewByHotelId({ hotelId: params.hotelId, limit: 2, page: page })
